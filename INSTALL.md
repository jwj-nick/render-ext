...
파일이나 폴더를 Chrome에서 열면 **뷰어 셸**로 전환됩니다: 왼쪽 사이드바는 계속 유지되고,
오른쪽에는 마지막으로 연 파일이 표시됩니다.

| 기능 | 방법 |
|---|---|
| **폴더 따라가기** | 사이드바 **Files**에서 폴더 클릭 → **왼쪽 목록만 바뀌고 오른쪽 뷰어는 그대로**. 상위 폴더(⬆)도 목록에 |
| **파일 열기** | Files에서 파일 클릭 → 오른쪽이 그 파일로 교체(페이지 이동 없음). **HTML은 새 탭**(브라우저가 렌더) |
| **목차** | Markdown이면 **Contents** 탭 → 클릭 스크롤 + 현재 위치 하이라이트 |
| 실패 시 | 없는 파일/렌더 실패면 사이드바는 유지되고 오른쪽에 실패 메시지 |
| 렌더 ↔ 원문 | 우상단 pill의 **Raw** 버튼 |
| 사이드바 접기 | 상단 `⟨` (다시 `☰`로 열기) |
| 기능 on/off | 툴바의 render-ext 아이콘 → 전체/Markdown/코드/폴더 열기 개별 토글 |
| 다크모드 | OS 설정 자동 추종 |
